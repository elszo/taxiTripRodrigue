...
def preprocess_data(data):
    ...
    distance_haversine = haversine_array(data.pickup_latitude, data.pickup_longitude, data.dropoff_latitude,
                                         data.dropoff_longitude)
    data['log_distance_haversine'] = np.log1p(distance_haversine)
    data['is_high_traffic_trip'] = is_high_traffic_trip(data).astype(int)
    data['is_high_speed_trip'] = is_high_speed_trip(data).astype(int)
    data['is_rare_pickup_point'] = is_rare_point(data, "pickup_latitude", "pickup_longitude", 0.01, 0.995, 0, 0.95).astype(
        int)
    data['is_rare_dropoff_point'] = is_rare_point(data, "dropoff_latitude", "dropoff_longitude", 0.01, 0.995, 0.005,
                                                 0.95).astype(int)
    return data
